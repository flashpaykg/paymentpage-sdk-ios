...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        ecompaySDK.presentPayment(at: self, paymentInfo: paymentInfo) { (result) in
            print("flashpaySDK finished with status \(result.status.rawValue)")
...
//        theme.showDarkKeyboard = true
    ...
    static func handleErrorCode(from error: Error) {
        if let codeNumber = (error as NSError).userInfo[FlashpaySDK.kSDKInitErrorCodeKey] as? NSNumber,
